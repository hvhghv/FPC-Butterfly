...
- **效果序列**：每颗灯珠可编排最多 8 步的效果组合循环
  （如 1s 静态 → 1s 闪烁 → 1s 呼吸），**每步可独立指定颜色与亮度**
- **色轮选色**：灯珠颜色与序列每步颜色均可用 HSV 色轮（圆盘）选色，
  同时保留系统原生取色器与 RGB 滑条
...
每颗灯珠可编排最多 8 步的效果序列，**每步可独立指定颜色与亮度**，
实现「暗红呼吸 → 亮绿闪烁 → 蓝色彩虹」这类多彩循环。

#### 色轮选择器

点击任意颜色圆点即弹出 HSV 色轮（Canvas 绘制）：

- **圆盘选色**：角度 = 色相 H，半径 = 饱和度 S，中心为白、边缘为纯色
- **亮度滑条**：独立控制明度 V（0-100%）
- **十六进制输入**：可直接粘贴 `#RRGGBB`，实时联动
- 支持鼠标拖动与触摸，`ESC` 或点击遮罩关闭

灯珠主颜色同时提供三种选色方式：RGB 滑条、色轮圆点、系统原生取色器。

#### 步骤颜色控制

| 操作 | 效果 |
|------|------|
| 点击步骤的颜色圆点 | 打开色轮选色，选定后该步使用此颜色 |
| 右键点击颜色圆点 | 清除该步颜色，回到沿用灯珠全局颜色 |
| 拖动步骤亮度滑条 | 自动启用该步颜色并设置独立亮度 |

> 未指定颜色的步骤会沿用灯珠的全局颜色与亮度，
> 因此旧配置导入后行为完全不变。

#### 数据格式

```json
{"cmd":"led.sequence","id":0,"steps":[
  {"effect":"breath","duration":1000,"period":0,
   "r":255,"g":0,"b":0,"brightness":80},
  {"effect":"blink","duration":1000,"period":0,
   "r":0,"g":255,"b":0,"brightness":255}
]}
```

`r`/`g`/`b` 三通道齐全时该步使用指定颜色与 `brightness`；
缺省则沿用灯珠全局设置。

> NVS 存储版本已升至 `LED_NVS_VERSION = 2`，
> 配置导出格式版本升至 `CFG_VERSION = 2`（新增步骤颜色/亮度字段）。
